 ...
 * - 인메모리 5곡+ 프레젠테이션 데이터를 기반으로 완전 오프라인(Zero-Fetch)으로 동작
...
export function FullscreenPresentRoute(): React.JSX.Element {
  ...
  // Fullscreen 상태 모니터링: 전체화면이 해제되면 즉시 송출을 종료 (주소창이 보이는 창 모드 슬라이드쇼 원천 방지)
  useEffect(() => {
    // 마운트 시 자동 전체화면 진입 시도
    ...
    const handleFullscreenChange = () => {
      ...
      if (inFullscreen) {
        ...
        // Esc 키 등으로 전체화면에서 벗어난 경우 주소창이 노출되는 창 모드로 머무르지 않고 즉시 송출 종료
        handleExit();
      }
    };
