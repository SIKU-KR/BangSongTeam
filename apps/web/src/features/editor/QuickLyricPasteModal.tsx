...
export function QuickLyricPasteModal({
  ...
  return (
    ...
    >
      <div className="relative flex flex-col w-full max-w-4xl max-h-[90vh] bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl text-zinc-100 overflow-hidden">
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-zinc-900/80">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              가사를 붙여넣으면 빈 줄 기준으로 슬라이드가 자동 분할됩니다. (최대 4줄 제한)
            </p>
          </div>
          <button
            type="button"
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex-1 flex flex-col min-h-[220px]">
              <label
                ...
                className="block text-xs font-medium text-zinc-300 mb-1"
              >
                가사 원문 <span className="text-red-400">*</span>
              </label>
              <textarea
                ...
                className="w-full flex-1 min-h-[220px] p-3 bg-zinc-800/80 border border-zinc-700 rounded-lg text-sm text-zinc-100 placeholder-zinc-500 font-mono leading-relaxed resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
              />
            </div>
          </div>

          {/* Right Column: Real-time Slide Cards Preview */}
          <div className="flex flex-col bg-zinc-950/60 border border-zinc-800 rounded-lg p-4 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-3 pr-1">
              {slides.length === 0 ? (
                <div className="h-48 flex flex-col items-center justify-center text-center text-zinc-500 text-xs p-4">
                ...
                slides.map((slide) => (
                  <div
                    ...
                    className="p-3 bg-zinc-800/60 border border-zinc-700/60 rounded-lg hover:border-zinc-600 transition-colors"
                  >
                    ...
                    <div className="text-xs text-zinc-200 space-y-1 font-sans">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-zinc-800 bg-zinc-900/80">
          ...
          <button
            ...
            onClick={handleAdd}
            disabled={!isValid}
            className="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-500 disabled:bg-zinc-800 disabled:text-zinc-500 disabled:cursor-not-allowed rounded-lg shadow-sm transition-colors"
          >
            세트에 추가
          </button>
        </div>
