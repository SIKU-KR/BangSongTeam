...
```
src/client/   React SPA: routes/, features/<area>/, components/{ui,common,layout,stage}/, lib/{api,auth,storage,sync,offline}
...
config/       drizzle.config.ts (passed to drizzle-kit by path)
```
...
- A config file stays at the root only if a tool finds it there by name (`components.json` is one). Tailwind has no config file: tokens live in `src/client/index.css`. Don't add `postcss.config.js`, `tailwind.config.*` or extra `vitest.*.config.ts` files; extend the configs inlined in `vite.config.ts` and `vitest.config.ts`.
