...
- **Client**: React 19, React Router 7 in library mode (not Next.js or TanStack Start), TanStack Query, Tailwind CSS 3 with hand-written components (shadcn/ui and Radix are not installed)
...
```
src/client/   React SPA: routes/, features/<area>/, components/stage/ (3-layer stage), lib/{api,auth,storage,sync,offline}
...
src/db/       "#db": Drizzle schema, queries/ (all DB access), ops/ (runbook SQL). Worker-only
migrations/   D1 SQL generated by drizzle-kit, plus hand-written FTS5 SQL
tests/        Repo-level config tests (package.json, wrangler, PWA, migrations)
config/       drizzle.config.ts, tailwind.config.js (passed to tools by path)
```

- Cross-layer imports use the `package.json` subpath imports `#shared` and `#db`. There are no path aliases.
- A config file stays at the root only if a tool finds it there by name. Don't add `postcss.config.js` or extra `vitest.*.config.ts` files; extend the configs inlined in `vite.config.ts` and `vitest.config.ts`.
- Older docs (especially `docs/tasks/**`) use the pre-2026-09-24 paths: `apps/web/src` → `src/client`, `apps/web/worker` → `src/worker`, `packages/shared/src` / `@repo/shared` → `src/shared` / `#shared`, `packages/db/src` / `@repo/db` → `src/db` / `#db`, `packages/db/drizzle` → `migrations`.
...
```bash
pnpm install
cp .dev.vars.example .dev.vars     # local secrets; DEV_LOGIN_ENABLED=true turns on dev login
...
| `node`   | `src/shared/**`, `src/db/**`, `tests/**` | Node; `src/db` tests use an in-memory SQLite from `createTestDb()` |
...
- Some tests guard files outside their own folder and fail when you touch them:
  - `tests/config.test.ts`: `package.json` scripts and imports, `wrangler.jsonc`
  - `tests/pwaConfig.test.ts`: the PWA config
  - `tests/migrations.test.ts`: no `backgrounds` rows in migrations
...
- All history is squashed into `migrations/0001_initial.sql` (journal `idx: 1`), so the next `pnpm db:generate` writes `0002_*`. A D1 that applied older files must be recreated. Locally, delete `.wrangler/state/v3/d1` and run `pnpm db:migrate:local`.
