...
- [x] **Task 3.1: 송출 위치 계산 순수 함수 (TDD)**
  - **대상 파일**: `src/client/features/presentation/projectionState.ts`
  - **선행 조건**: 없음
  - **구현 내용**:
  ...
  - **DoD (통과 기준)**: `pnpm exec vitest run src/client/features/presentation/projectionState.test.ts`가 100% 통과(Green)한다.

- [x] **Task 3.2: BroadcastChannel 동기화 훅 (TDD)**
  - **대상 파일**: `src/client/features/presentation/useProjectionChannel.ts`
  ...
  - **DoD (통과 기준)**: `pnpm exec vitest run src/client/features/presentation/useProjectionChannel.test.ts`가 100% 통과(Green)한다.
...
- [x] **Task 3.3: 전체화면 송출 라우트를 순수 함수 기반으로 리팩터링**
  ...
  - **DoD (통과 기준)**: `pnpm exec vitest run src/client/routes/FullscreenPresentRoute.test.tsx`가 100% 통과(Green)한다.
...
- [x] **Task 3.4: 청중 모드(`?audience=1`) 추가**
  ...
  - **DoD (통과 기준)**: `pnpm exec vitest run src/client/routes/FullscreenPresentRoute.audience.test.tsx`가 100% 통과(Green)한다.
