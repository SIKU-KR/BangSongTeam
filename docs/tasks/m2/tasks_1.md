...
- **단일 원천 상태(Single Source of Truth)**: 곡(Deck)과 세트(Presentation)의 데이터 모델 및 스타일은 `@repo/shared`의 Zod 스키마(`DeckStyleSchema`, `SlideSchema`, `PresentationSchema`)를 엄격히 준수하며 수동 interface를 중복 생성하지 않는다.
