...
3. **PRD 마일스톤 로드맵 매핑**:
   ...
   - `M3-B`: 계정 및 서버 저장 (Hono RPC 클라이언트, 덱/프레젠테이션 D1 CRUD, Better Auth 카카오/네이버, 가사 버전 기여)
...
### 2.1 현재 위치 (2026-09-21)

| 마일스톤  | 상태                                                                                                                                           |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| M0        | 완료                                                                                                                                           |
| M1        | 코드 완료 · 실사용 검증 대기 (저장 기능이 없어 검증 보류)                                                                                      |
| M2        | 잔여 3건 — 넘침 감지(`m2/tasks_1.md` 1.2·1.3), 슬라이드 나누기·합치기(`m2/tasks_2.md` 2.3·2.4), 15분 워크플로 통합 테스트(`m2/tasks_5.md` 5.5) |
| **M3-A**  | **다음 작업 — `m3/tasks_1.md`**                                                                                                                |
| M3-B 이후 | 대기. 태스크 문서는 착수 직전에 작성한다                                                                                                       |
...
> **영속성 단계 주의 (2026-09-21):** `worship-offline-db`는 아직 존재하지 않는다. 현재 데이터 원천은 `presentationStore`의 인메모리 상태이며(TECH_SPEC §5.5 Phase 1), M3-A에서 IndexedDB로 승격한다. 그전까지 "IndexedDB에서 로드"는 목표 상태를 가리키는 규칙이지 현재 코드의 설명이 아니다.
