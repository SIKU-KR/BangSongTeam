...
| `src/db` Drizzle 스키마·`src/db/migrations/`(§4) | 구현 | `0001_initial`(첫 배포 전 0000~0008을 합침, 2026-09-24), `0002_drop_user_backgrounds`, `0003_presentation_link_share`. 다음 마이그레이션은 0004부터                                                                                                                               |
...
### 4.1.1 배경 행은 마이그레이션에 넣지 않는다 (`src/db/migrations/0001_initial.sql`)
...
- **배경 행은 R2 객체가 올라간 뒤에만 만든다.** 사전 주입 배경은 `docs/ops/background-runbook.md` 절차(R2 `object put` → D1 `INSERT`)로 등록한다. 문장의 정본은 `src/db/ops/backgroundSql.ts`이고, `backgroundSql.test.ts`가 실제 스키마에 대해 실행하며 `runbook.test.ts`가 문서와 대조한다. `src/db/migrations.test.ts`는 `0001`에 배경 `INSERT`가 다시 들어오지 못하게 막는다.
...
### 4.2 FTS5 Trigram 검색 가상 테이블 (`src/db/migrations/0001_initial.sql`)
...
| `GET`    | `/api/share/:token`                  | 로그인 없이 링크로 세트 보기 (멤버 기록 없음)             | No           | 구현 |
...
- 로그인하지 않은 사람은 `GET /api/share/:token`으로 본다. 멤버 행을 만들지 않고 `access: { ownerName }`(memberId 없음)을 단 문서만 준다(`cache-control: private, no-store`). 클라이언트는 이 문서를 메모리에만 넣고(`showSharedPreview`) IndexedDB에 쓰지 않는다. '사본 만들기'는 `/s/:token?copy=1`로 로그인 화면을 띄우고, 로그인 뒤 같은 주소에서 `join`한 다음 편집기의 사본 만들기 창을 연다.
...
- **세트 링크 공유**: 링크(`/s/:token`)는 로그인 없이 보기 전용으로 열린다 (2026-09-25 결정). 아래 첫 슬라이드 원칙의 예외다: 링크는 소유자가 켠 세트에만 있고, 토큰은 추측할 수 없는 21자 NanoID이며, 소유자가 끄거나 재설정하면 곧바로 막힌다. 검색·목록으로 드러나지 않아 크롤링 대상이 아니다. 사본 만들기(멤버 기록 포함)는 로그인해야 한다.
