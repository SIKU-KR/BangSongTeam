...
module.exports = tseslint.config(
  ...
  // 계층(TanStack Query)과 API 클라이언트를 직접 import하지 않는다. 송출 중 API·데이터
  // 요청 0건 불변식을 import 수준에서 막는다. 배경 미디어를 조용히 받아 두는
  // 백그라운드 캐시(`lib/offline`)만 예외다 (2026-09-24).
  //
        ...
        "error",
        {
          paths: [
            ...
            },
            {
              ...
              message:
                "송출 화면은 API·데이터 요청 0건이어야 한다 (Zero-Fetch). 서버 캐시 훅을 쓰지 않는다.",
            },
          ],
          patterns: [
            {
            ...
            },
            {
              regex: "(^|/)lib/api(/|$)",
              message:
                "송출 화면은 API·데이터 요청 0건이어야 한다 (Zero-Fetch). API 클라이언트를 import하지 않는다.",
            },
          ],
        },
      ],
